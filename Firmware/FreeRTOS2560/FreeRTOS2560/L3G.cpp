...
/*char debugBufferl[1024];
void debugPrintl(const char *str)
{
	Serial.println(str);
	Serial.flush();
}
void dprintfl(const char *fmt, ...)
{
	va_list argptr;
	va_start(argptr, fmt);
	vsprintf(debugBufferl, fmt, argptr);
	va_end(argptr);
	debugPrintl(debugBufferl);
}*/
...
void L3G::read()
{
  ...
  {
	// dprintfl("%d",Wire.available());
	// dprintf("%d", getTimeout());
	//dprintfl("io %d", io_timeout);
